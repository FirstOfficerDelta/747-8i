...
var MG_steer = {
	...
	m.push = props.globals.initNode("sim/model/autopush/connected",0,"BOOL");
...
var nosmoking_knob = props.globals.initNode("controls/switches/nosmoking-sign",0,"INT");
var nosmoking_on = props.globals.initNode("controls/cabin/nosmoking-sign",0,"BOOL");

setlistener("controls/switches/nosmoking-sign", func {
    if (nosmoking_knob.getValue() == 0)
	nosmoking_on.setBoolValue(0);
    if (nosmoking_knob.getValue() == 1)
	nosmoking_on.setBoolValue(1);
    if (nosmoking_knob.getValue() == 2)
	nosmoking_on.setBoolValue(1);
},0,0);
...
#### update ####
update : func {
  if (me.cutoff.getBoolValue() == 1) me.started.setBoolValue(0);
  if (me.running.getBoolValue() ==1 and !me.started.getBoolValue() ==1) me.running.setBoolValue(0);

  if (me.fuel_out.getBoolValue() ==1) me.started.setBoolValue(0);
  if (me.started.getBoolValue() ==1) {
    ...
    me.throttle_lever.setValue(me.throttle.getValue());
      ...
      if (me.starter.getBoolValue() ==1) {
        me.spool_up();
          ...
          if (tmprpm > 0.0) {
            tmprpm -= getprop("sim/time/delta-realtime-sec") * 1.5;
            me.rpm.setValue(tmprpm);

            tmpn1ind -= getprop("sim/time/delta-realtime-sec") * 1.5;
            me.n1ind.setValue(tmpn1ind);

            tmpn2ind -= getprop("sim/time/delta-realtime-sec") * 1.5;
      ...
      spool_up : func {
        if (!me.cutoff.getBoolValue() ==1) {
          if (me.n1ind.getValue() < 20) {
            settimer(func {me.starter.setBoolValue(0);},0.2);

          } elsif (me.n1ind.getValue() >= 20 and me.n1ind.getValue() < me.n1.getValue()) {
            var tmpn1ind = me.n1ind.getValue();
            tmpn1ind += getprop("sim/time/delta-realtime-sec") * 1;
            me.n1ind.setValue(tmpn1ind);

            var tmpn2ind = me.n2ind.getValue();
            tmpn2ind += getprop("sim/time/delta-realtime-sec") * 1;
            me.n2ind.setValue(tmpn2ind);

            var tmprpm = me.rpm.getValue();
            tmprpm += getprop("sim/time/delta-realtime-sec") * 1;
            me.rpm.setValue(tmprpm);
            ...
            settimer(
              func {me.starter.setBoolValue(0);},1);
          }
    } else {
        var tmpn1ind = me.n1ind.getValue();
        ...
        if (tmprpm >= 25) {
          ...
          var tmpn1ind = me.n1ind.getValue();
          me.n1ind.setValue(25);

          var tmpn2ind = me.n2ind.getValue();
          me.n2ind.setValue(25);
